(ns jackdaw.serdes.avro.confluent-test
...
(def +registry+
  (merge jsac/+base-schema-type-registry+
         jsac/+UUID-type-registry+))

(def schema-type
  (jsac/make-coercion-stack
   +registry+))
...
(defn ->serde [schema-str]
  (let [schema-registry-config
        ...
        serde-config
        {:jsac/schema schema-str
         :key?        false}]
    (jsac/serde +registry+ schema-registry-config serde-config)))
...
(deftest schema-type-test
  (testing "schemaless"
    (is (= (jsac/clj->avro (schema-type nil) "hello" [])
           "hello"))
    (is (= 1 (jsac/avro->clj (schema-type nil) 1))))
  (testing "boolean"
    (let [avro-schema (parse-schema {:type "boolean"})
          schema-type (schema-type avro-schema)
          clj-data true
          avro-data true]
      (is (= clj-data (jsac/avro->clj schema-type avro-data)))
      (is (= avro-data (jsac/clj->avro schema-type clj-data [])))))
  (testing "double"
  ...
  (testing "int"
    (let [avro-schema (parse-schema {:type "int"})
          ...
          clj-data (int 2)
          avro-data 2]
      (is (jsac/match-clj? schema-type clj-data))
      (is (= clj-data (jsac/avro->clj schema-type avro-data)))
      (is (= avro-data (jsac/clj->avro schema-type clj-data [])))))
  (testing "long"
    (let [avro-schema (parse-schema {:type "long"
      ...
      (is (int? (jsac/clj->avro schema-type (byte clj-data) [])))
      (is (int? (jsac/clj->avro schema-type (short clj-data) [])))
      (is (int? (jsac/clj->avro schema-type (int clj-data) [])))))
  ...
  (testing "unmarshalling a utf8 character set"
    (let [avro-schema (parse-schema {:namespace "com.fundingcircle"
          ...
          utf8 (Utf8. b)]
      (is (= (String. b) (jsac/avro->clj schema-type utf8)))))
  (testing "null"
  ...
  (testing "array"
    (let [avro-schema (parse-schema {:namespace "com.fundingcircle"
          ...
          avro-data (GenericData$Array. ^Schema avro-schema
                                        ^Collection clj-data)]
      (is (jsac/match-clj? schema-type clj-data))
      (is (jsac/match-clj? schema-type (seq clj-data)))
      (is (= clj-data (jsac/avro->clj schema-type avro-data)))
      (is (= avro-data (jsac/clj->avro schema-type clj-data [])))))
  (testing "nested array"
    (let [nested-schema-json {:name "nestedRecord"
      ...
      (is (jsac/match-clj? schema-type clj-data))
      (is (jsac/match-clj? schema-type {:stringField "foo"
                                        :longField 123
                                        :recordField [{:b 1}]}))
  ...
  (testing "map"
    (let [nested-schema-json {:name "nestedRecord"
          ...
          avro-data-str-keys (reduce-kv (fn [acc k v]
                                        ...
                                        avro-data)]
      (is (= clj-data (jsac/avro->clj schema-type avro-data)))
      (is (= avro-data-str-keys (jsac/clj->avro schema-type clj-data [])))))
  (testing "union"
    (let [avro-schema (parse-schema ["long" "string"])
      ...
      (is (= clj-data-long (jsac/avro->clj schema-type avro-data-long)))
      (is (= avro-data-long (jsac/clj->avro schema-type clj-data-long [])))
      (is (= clj-data-string (jsac/avro->clj schema-type avro-data-string)))
      (is (= (str avro-data-string) (jsac/clj->avro schema-type clj-data-string [])))))
  ...
  (testing "enum"
    (let [enum-schema {:type "enum"
      ...
      (is (= clj-data (jsac/avro->clj schema-type avro-data)))
      (is (= avro-data (jsac/clj->avro schema-type clj-data [])))
      (is (= avro-data (jsac/clj->avro schema-type {:industry-code-version "SIC-2003"} [])))))
  (testing "record"
    (let [nested-schema-json {:name "nestedRecord"
                              :type "record"
                              :fields [{:name "a"
      ...
      (is (jsac/match-clj? schema-type clj-data))
      (is (jsac/match-clj? schema-type clj-data-opt))
      (is (not (jsac/match-clj? schema-type (assoc clj-data-opt :optionalField (inc (long Integer/MAX_VALUE))))))
      (is (not (jsac/match-clj? schema-type (assoc clj-data-opt :optionalField (dec (long Integer/MIN_VALUE))))))
      (is (= (assoc clj-data :optionalField nil :defaultField 1) (jsac/avro->clj schema-type avro-data)))
      (is (= avro-data (jsac/clj->avro schema-type clj-data [])))
      (is (instance? Integer (.get (jsac/clj->avro schema-type clj-data-opt []) "optionalField")))))
  (testing "marshalling record with unknown field triggers error"
    (let [avro-schema (parse-schema {:type "record"
      ...
      (is (thrown-with-msg? clojure.lang.ExceptionInfo
                            ...
                            (jsac/clj->avro schema-type {:garbage "yolo"} [])))))
  (testing "uuid"
    (let [avro-schema (parse-schema {:type "string",
                                     :logicalType "uuid"})
          schema-type (schema-type avro-schema)
